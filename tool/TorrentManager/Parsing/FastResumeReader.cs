...
internal static class FastResumeReader
{
    ...
    private static byte[] GetRequiredBytes(Dictionary<string, object> dict, string key)
    {
        if (!dict.TryGetValue(key, out var value) || value is not byte[] data)
        {
            throw new InvalidDataException($"缺少或无效字段: {key}");
        }
    ...
    private static string NormalizeInfoHash(byte[] raw)
    {
        ...
        if (text.Length == 40 && text.All(IsHexChar))
        {
